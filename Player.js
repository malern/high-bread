class Player {
	constructor(mechanics) {
		...
		this.body = Matter.Bodies.rectangle(500, 200, 150, 150, {
			render: {
				strokeStyle: '#ffffff',
				sprite: {
					texture: 'bread.png'
			...
			// TODO: remove this once findClosestIntersect no longer relies on it
			isPlayer: true,
		});
	...
	grappleTo(grappleId, targetPoint) {
		// grapple
		var endRopePoint = Matter.Vector.add(Matter.Vector.mult(Matter.Vector.normalise(Matter.Vector.sub(targetPoint, this.body.position)), 5000), this.body.position);

		var closestIntersect = findClosestIntersect(this.body.position, endRopePoint, this.mechanics.engine.world.bodies);
		canvasRenderer.closestIntersect = closestIntersect;
		console.log(closestIntersect);

		if (this.grappleConstraints[grappleId]) {
			Matter.World.remove(this.mechanics.engine.world, this.grappleConstraints[grappleId]);
		...
		if (closestIntersect) {
			...
			Matter.World.addConstraint(this.mechanics.engine.world, this.grappleConstraints[grappleId]);
		}
	}
